...
class ParticleSystem {
    ...
    private renderQuadGeometryVBO(context: __esri.RenderContext) {
        const gl = context.gl;
    ...
    update(context: __esri.RenderContext): void {
        ...
        gl.viewport(0, 0, context.camera.viewport[2], context.camera.viewport[3]);
    ...
    private renderParticles(context: __esri.RenderContext): void {
        const gl = context.gl;
...
class ExternalRenderer {
    ...
    constructor(private view: __esri.SceneView) {
        view.on("hold", () => {
    ...
    renderTransparent(context: __esri.RenderContext) {
    ...
    private prepareResources(context: __esri.RenderContext) {
        let rasterInfo: FetchedRaster;

        return this.fetchRaster()
            .then((fetchedRaster: any) => {
    ...
    private createParticleSystem(context: __esri.RenderContext, extent: Extent) {
    ...
    private createTextures(context: __esri.RenderContext, fetchedRaster: FetchedRaster) {
        // Create:
     ...
     */
    private createTexture(gl: WebGL2RenderingContext, size: number, data: Uint8Array | HTMLImageElement, interpolation: number) {
        const texture = gl.createTexture();
     ...
     */
    private fetchRaster(): Promise<FetchedRaster> {
        ...
        return request(`./assets/wind-global.png`, requestOptions)
            .then((response: any) => {
                return {
                    serviceInfo,
                    extent,
